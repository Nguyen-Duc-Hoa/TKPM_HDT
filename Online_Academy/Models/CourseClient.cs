...
namespace Online_Academy.Models
{
    public class CourseClient
    {
        ...
        //Lay ds Course chua mua
        public IEnumerable<Course> GetCourse_notBought(int idUser)
        {
            try
            {
                HttpClient client = new HttpClient();
                client.BaseAddress = new Uri(Base_URL);
                client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                HttpResponseMessage respone = client.GetAsync("Course_User/notBought/" + idUser).Result;
                if (respone.IsSuccessStatusCode)
                    return respone.Content.ReadAsAsync<IEnumerable<Course>>().Result;
                return null;
            }
